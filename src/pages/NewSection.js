...
export default function NewSection() {
    ...
    const formatNumber = (number) => {
        return `${number ? number.toFixed(2) : 0}`.split(/(?=(?:\d{3})+$)/).join(',');
    }
    ...
    useEffect(() => {
        ...
        const calculatedCashSavings = purchaseAmount * taxBrack;
        ...
        setFirstYearDeduction(formatNumber(calculatedDeduction + calculatedBonus));
